          ...
          {items?.map((s, i) => (
            ...
            >
              ...
              <div className='pt-6 border-t border-white/5'>
                <Button asChild variant='ghost' className='w-full justify-between hover:bg-zebotix-blue hover:text-white group/btn'>
      ...
      <div className='section-container'>
        <div className='mb-12'>
          ...
          <Reveal delay={0.2}>
            <p className='text-gray-400 text-lg max-w-3xl'>
              Practical digital infrastructure designed to dominate markets. 
              From niche e-commerce to enterprise ERPs, we build the tools that power growth.
            </p>
          </Reveal>
        </div>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4'>
            {PLATFORMS.map((p, i) => (
              <Reveal key={p.id} delay={0.05 * i} distance={20}>
                <div
                  className='bg-zebotix-darkGray rounded-xl p-5 border border-gray-800 text-gray-300 hover:border-zebotix-blue/30 hover:bg-zebotix-blue/5 transition-all duration-300 flex items-center justify-between group'
                >
                  <span className='font-medium'>{p.title}</span>
                  <ArrowRight className='h-4 w-4 opacity-0 group-hover:opacity-100 transform translate-x-[-10px] group-hover:translate-x-0 transition-all text-zebotix-blue' />
                </div>
              </Reveal>
            ))}
          </div>
        ...
        <Reveal delay={0.6} className='mt-16 text-center'>
          <div className='p-12 rounded-3xl bg-linear-to-r from-blue-900/20 to-transparent border border-blue-500/10'>
            ...
            <Button asChild size='lg' className='bg-zebotix-blue hover:bg-blue-600 px-10 h-14 text-lg font-semibold'>
              <Link href='/contact'>
                Talk to an Expert
              </Link>
            </Button>
